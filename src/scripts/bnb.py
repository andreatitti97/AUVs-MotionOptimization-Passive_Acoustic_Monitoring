...
class BinaryKnapsack(pybnb.Problem):
    def __init__(self, W, v, w):
        ...
        self._sorted_order = sorted(
            range(self._n), key=lambda i: self._v[i] / float(self._w[i]), reverse=True
        )
        self._weight = 0
        self._value = 0
        self._level = 0
    ...
    def save_state(self, node):
        node.state = (self._weight, self._value, self._level, self._choices)

    def load_state(self, node):
        (self._weight, self._value, self._level, self._choices) = node.state
...
if __name__ == "__main__":
    ...
    W = 25 #knapsnack capacity
    w = [2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2] # weights 
    v = [
        205,
        206,
        207,
