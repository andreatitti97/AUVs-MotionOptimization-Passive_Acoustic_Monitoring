...
def run_simulation(robots, tracker1, sensor1, sensor2, sensor3, sensor4, pub_estimation, pub_platform_state, pub_covariance):
    ...
    while t <= TIME_DURATION:
        ...
        # SIMULATE SENSORS MEASURAMENTS
            ...
            sensor3.vehiclePose(instance.pose.x,instance.pose.y,instance.pose.theta)
            sensor3.targetPoseReal(instance.pose_target.x,instance.pose_target.y,instance.pose_target.theta)
            [measure3, vehicle_pose3, rel_bearing3] = sensor3.measureBearing()  
        
            sensor4.vehiclePose(instance.pose.x,instance.pose.y,instance.pose.theta)
            sensor4.targetPoseReal(instance.pose_target.x,instance.pose_target.y,instance.pose_target.theta)
            [measure4, vehicle_pose4, rel_bearing4] = sensor4.measureBearing()  
        ...
        if count1 % 5 or count1 == 1: #TODO update EKF not always
            if count1 == 1:
                tracker1.processMeasurement(measures,initial_guess, vehicle_pose, vehicle_pose2,vehicle_pose3,vehicle_pose4, TIME_SCALER*TIME_STEP) #FIRST UPDATE
            tracker1.processMeasurement(measures,initial_guess, vehicle_pose, vehicle_pose2,vehicle_pose3,vehicle_pose4, EKF_MEAS_UPDATE*TIME_SCALER*TIME_STEP)#update EKF with a measurament each 2 sec
        ...
        auv3_x.append(vehicle_pose3[0])
        auv3_y.append(vehicle_pose3[1])
        auv4_x.append(vehicle_pose4[0])
        auv4_y.append(vehicle_pose4[1])
...
def main():
    ...
    sensor1 = sensor.Sensor('first_streamer',1,0,MEAS_VARIANCE,1,0,BASELINE_Y)#freq,mean,variance,displachement
    sensor2 = sensor.Sensor('seconda_streamer',1,0,MEAS_VARIANCE,-1,0,BASELINE_Y)
    sensor3 = sensor.Sensor('third_streamer',1,0,MEAS_VARIANCE, 1,BASELINE_X,BASELINE_Y)
    sensor4 = sensor.Sensor('fourth_streamer',1,0,MEAS_VARIANCE,-1,BASELINE_X,BASELINE_Y)
    ...
    run_simulation(robots, tracker1, sensor1, sensor2, sensor3, sensor4, pub_estimation, pub_platform_state, pub_covariance)
