...
def run_simulation(robots, obs, auv, pub, poly_traj):
    ...
    while t <= config.TIME_DURATION:
        ...
        if count1 % 5 or count1 == 1: # each (12 s) and at (0.8 s) first update
            if count1 == 1:
                delta_meas = config.TIME_SCALER*config.TIME_STEP
            else:
                delta_meas = config.EKF_MEAS_UPDATE*config.TIME_SCALER*config.TIME_STEP
            for i in range(len(auv)):
        ...
        if count1 % 50: #TODO update EKF not always
            for i in range(len(auv)):
                obs[i].processMeasurement(measures,initial_guess, vehicle_pose, 
                    10*config.EKF_MEAS_UPDATE*config.TIME_SCALER*config.TIME_STEP, True) #DISTRIBUTED UPDATE
        ...
        if count1%((config.N*config.OPTIMIZATION_TIME_STEP)/(config.TIME_STEP*config.TIME_SCALER)) == 0 and config.OPTIMIZATION_ON == True: 
...
def main():
    ...
    tracker2 = tracker.Tracker('second_observer',False, config.N_AUV)
    tr.append(tracker1)
    tr.append(tracker2)
    if config.N_AUV > 2:
        tracker3 = tracker.Tracker('third_observer',False, config.N_AUV)
        tracker4 = tracker.Tracker('forth_observer',False, config.N_AUV)
        tr.append(tracker3)
        tr.append(tracker4)
    ...
    robot_1 = robot.Robot("platoform_center", "y", controller1_auv, controller1_target)
    auv = []
