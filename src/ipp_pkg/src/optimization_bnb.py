...
target_theta = TARGET_INIT[2] #non serve toglila
# Sensors
...
def simulation(control_input, target_est, platform_pose, P):
    ...
    for t in range(0,int(tc/4)):
        ...
        #update measurament
        sensor1.vehiclePose(platform_state[0], platform_state[1], platform_state[2], TIME_STEP*400)  
        sensor1.targetPoseReal(target_state[0], target_state[1], target_theta)
        
        sensor2.vehiclePose(platform_state[0], platform_state[1], platform_state[2], TIME_STEP*400)
        ...
        tracker_.processMeasurement(measures,target_state, sensor_pose1, sensor_pose2, TIME_STEP*400)
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        P = np.matrix([[200, 0, 0, 0], #TODO INITIAL COV VALUE AFTER LAST ESTIMATE - TO CHECK
                        [0, 200, 0, 0],
                        [0, 0, t_est[2], 0],
                        [0, 0, 0, t_est[3]]])
