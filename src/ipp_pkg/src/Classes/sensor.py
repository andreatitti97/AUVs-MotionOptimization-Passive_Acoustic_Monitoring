...
class Sensor:
    ...
    def vehiclePose(self, x_v, y_v, theta_v, dt):#w.r.t the <w> - return auv pose from refernce pose
        ...
        self.w_pose_s = np.array([(x_v+np.sin(self.theta_v)*self.baseline/2)+np.cos(self.theta_v)*self.lin_vel*dt,
                                    (y_v-np.cos(self.theta_v)*self.baseline/2)+np.sin(self.theta_v)*self.lin_vel*dt,
                                        self.theta_v])
