...
spec = importlib.util.spec_from_file_location("module.dekf", "/home/andrea/ros_simulation_ws/src/ipp_pkg/src/Classes/EKF_distributed.py")
...
class Tracker:
    ...
    '''
    def __init__(self, id, bool, n_auv, init_cov=[]):

        self.id  = id
        self.n_auv = n_auv
        if bool == True:
            self.__ekf = dekf.ExtendedKalmanFilter(n_auv,bool,init_cov)
        else:
            self.__ekf = dekf.ExtendedKalmanFilter(n_auv,bool)
