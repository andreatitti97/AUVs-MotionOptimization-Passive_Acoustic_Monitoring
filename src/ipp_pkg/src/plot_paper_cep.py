...
plt.yticks(fontsize=25, rotation=0)#to s25
plt.show()
...
if len(t) >= len(auv2_x_on):
    n = len(auv4_x_on)
elif len(t) < len(auv2_x_on):
    n = len(t)

t_off = np.linspace(0,config.TIME_DURATION,len(auv2_x_off))
t_on = np.linspace(0,config.TIME_DURATION,len(auv2_x_on))
baseline_angle = []
