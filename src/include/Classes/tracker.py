from dekf import ExtendedKalmanFilter
...
class Tracker:
    ...
    def processMeasurement(self,measures, target_init, sensors_state, tc):
        ...
        #ctrl_cmd = self.__ekf.update(measures, sensors_state, target_init, vehicle_pose)
        self.__ekf.update(measures, sensors_state)
        
        #print('trackerID:',self.id, self.__ekf.current_estimate)
        #5th COMMUNICATION 
        #TODO - COMMUNICATION AND FUSION (consensus)
        '''
        HERE COMMUNICATION OF STATE ESTIMATED AND COV BETWEEN THE 2 TRACKERS
        q1 = Queue()

        t1 = Thread(target = consumer, args =(q1, ))
        t2 = Thread(target = producer, args =(q1,self.__ekf.current_estimate))
